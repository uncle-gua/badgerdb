...
// Key is shorthand for specifying a query to run again the Key in a badgerdb, simply returns ""
// Where(badgerdb.Key).Eq("testkey")
const Key = ""

// Query is a chained collection of criteria of which an object in the badgerdb needs to match to be returned
...
// Where starts a query for specifying the criteria that an object in the badgerdb needs to match to
// be returned in a Find result
/*
Query API Example

	s.Find(badgerdb.Where("FieldName").Eq(value).And("AnotherField").Lt(AnotherValue).
		Or(badgerdb.Where("FieldName").Eq(anotherValue)
...
func Where(field string) *Criterion {
	if !startsUpper(field) {
		panic("The first letter of a field in a badgerdb query must be upper-case")
	}
...
func (q *Query) validateIndex(data interface{}) error {
	...
	for i := 0; i < q.dataType.NumField(); i++ {
		if tag := q.dataType.Field(i).Tag.Get(badgerdbIndexTag); tag == q.index {
...
// Record is the complete record for a given row in badgerdb
...
func (s *Store) runQuery(tx *badger.Txn, dataType interface{}, query *Query, retrievedKeys KeyList, skip int,
	...
	for k, v := iter.Next(); k != nil; k, v = iter.Next() {
		...
		if hasKeyField {
			err = s.setKeyField(k, val, keyField, storer.Type())
			if err != nil {
				return err
			}
		}
