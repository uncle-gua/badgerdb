...
func TestInsert(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		if err != badgerdb.ErrKeyExists {
			t.Fatalf("Insert didn't fail! Expected %s got %s", badgerdb.ErrKeyExists, err)
		}
...
func TestUpdate(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		if err != badgerdb.ErrNotFound {
			t.Fatalf("Update without insert didn't fail! Expected %s got %s", badgerdb.ErrNotFound, err)
		}
	...
	for _, tst := range testResults {
		t.Run(tst.name, func(t *testing.T) {
			testWrap(t, func(store *badgerdb.Store, t *testing.T) {
				...
				if err != nil {
					t.Fatalf("Error updating data from badgerdb: %s", err)
				}

				var result []ItemTest
				err = store.Find(&result, badgerdb.Where("UpdateIndex").Eq("updated index").
					And("UpdateField").Eq("updated"))
				if err != nil {
					t.Fatalf("Error finding result after update from badgerdb: %s", err)
				}
...
func TestIssue14(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		err = store.Find(&result, badgerdb.Where("Category").Eq("Test Category"))
		if err != nil {
...
func TestIssue14UpdateMatching(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		err = store.UpdateMatching(&ItemTest{}, badgerdb.Where("Name").Eq("Test Name"),
...
func TestInsertSequence(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		type SequenceTest struct {
			Key uint64 `badgerdbKey:"Key"`
		}

		for i := 0; i < 10; i++ {
			err := store.Insert(badgerdb.NextSequence(), &SequenceTest{})
			if err != nil {
...
func TestInsertSequenceSetKey(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		type InsertSequenceSetKeyTest struct {
			// badgerdb.NextSequence() creates an auto-key that is a uint64
			Key uint64 `badgerdbKey:"Key"`
		}
		...
		for i := 0; i < 10; i++ {
			...
			err := store.Insert(badgerdb.NextSequence(), &st)
			if err != nil {
				t.Fatalf("Error inserting data for sequence test: %s", err)
			}
			if int(st.Key) != i {
...
func TestAlternateTags(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		type TestAlternate struct {
			Key  uint64 `badgerdb:"key"`
			Name string `badgerdb:"index"`
		}
		item := TestAlternate{
			Name: "TestName",
		}
		...
		err = store.Find(&result, badgerdb.Where("Name").Eq(item.Name).Index("Name"))
		if err != nil {
...
func TestUniqueConstraint(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		err := store.Insert(badgerdb.NextSequence(), item)
		if err != nil {
		...
		t.Run("Update", func(t *testing.T) {
			...
			err = store.Insert(badgerdb.NextSequence(), update)
		...
		t.Run("UpdateMatching", func(t *testing.T) {
			...
			err = store.UpdateMatching(TestUnique{}, badgerdb.Where(badgerdb.Key).Eq(update.Key),
...
func TestIssue46ConcurrentIndexUpdateMatching(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		for i := range testData {
			...
			go func(gt *testing.T, i int) {
				defer wg.Done()
				err := store.UpdateMatching(ItemTest{}, badgerdb.Where(badgerdb.Key).
...
func (i *CustomStorer) Indexes() map[string]badgerdb.Index {
	return map[string]badgerdb.Index{
		"Name": {
