...
func TestBadger(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		b := store.Badger()
		if b == nil {
			t.Fatalf("Badger is null in badgerdb")
		}
	})
}
...
func TestAlternateEncoding(t *testing.T) {
	...
	store.Find(&result, badgerdb.Where(badgerdb.Key).Eq(tData.Key))
...
func (i *ItemWithStorer) Indexes() map[string]badgerdb.Index {
	return map[string]badgerdb.Index{
		"Name": {
			IndexFunc: func(_ string, value interface{}) ([]byte, error) {
				...
				return badgerdb.DefaultEncode(v)
			},
			Unique: false,
		},
	}
...
func TestIssue70TypePrefixCollision(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		query := badgerdb.Where(badgerdb.Key).In(0, 1, 2, 3, 4)
...
func TestIssue71IndexByCustomName(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		var result []Person
		ok(t, store.Find(&result, badgerdb.Where("Division").Eq(record.Division).Index("IdxDivision")))
	})
}

// utilities

func testWrap(t *testing.T, tests func(store *badgerdb.Store, t *testing.T)) {
	testWrapWithOpt(t, testOptions(), tests)
}

func testWrapWithOpt(t *testing.T, opt badgerdb.Options, tests func(store *badgerdb.Store, t *testing.T)) {
	var err error
	store, err := badgerdb.Open(opt)
	if err != nil {
		t.Fatalf("Error opening %s: %s", opt.Dir, err)
...
func testOptions() badgerdb.Options {
...
func tempdir() string {
	name, err := os.MkdirTemp("", "badgerdb-")
	if err != nil {
		panic(err)
	}
	return name
