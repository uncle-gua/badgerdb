...
func (i *ItemTest) Compare(other interface{}) (int, error) {
	...
	return 0, &badgerdb.ErrTypeMismatch{Value: i, Other: other}
}
...
func TestFindWithComparer(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		err := store.Find(&result, badgerdb.Where("Inner").Gt(data[1].Inner))
		if err != nil {
	...
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		for i := 0; i < to.NumField(); i++ {
			...
			t.Run(fmt.Sprintf("Builtin type %s equal", to.Field(i).Name), func(t *testing.T) {
				...
				// equal
				var result []All
				err := store.Find(&result, badgerdb.Where(to.Field(i).Name).Eq(curField))
				if err != nil {
			...
			t.Run(fmt.Sprintf("Builtin type %s greater than", to.Field(i).Name), func(t *testing.T) {
				// gt
				var result []All
				err := store.Find(&result, badgerdb.Where(to.Field(i).Name).Gt(curField))
				if err != nil {
					t.Fatalf("Error finding equal result %s", err)
				}
			...
			t.Run(fmt.Sprintf("Builtin type %s less than", to.Field(i).Name), func(t *testing.T) {
				// lt
				var result []All
				err := store.Find(&result, badgerdb.Where(to.Field(i).Name).Lt(curField))
				if err != nil {
					t.Fatalf("Error finding equal result %s", err)
				}
