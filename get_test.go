...
func TestIssue36(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		data1 := []*Tag1{{}, {}, {}}
		for i := range data1 {
			ok(t, store.Insert(badgerdb.NextSequence(), data1[i]))
			equals(t, uint64(i), data1[i].ID)
		}

		data2 := []*Tag2{{}, {}, {}}
		for i := range data2 {
			ok(t, store.Insert(badgerdb.NextSequence(), data2[i]))
			equals(t, uint64(i), data2[i].ID)
		}

		data3 := []*Tag3{{}, {}, {}}
		for i := range data3 {
			ok(t, store.Insert(badgerdb.NextSequence(), data3[i]))
			equals(t, uint64(i), data3[i].ID)
		}

		data4 := []*Tag4{{}, {}, {}}
		for i := range data4 {
			ok(t, store.Insert(badgerdb.NextSequence(), data4[i]))
		...
		for i := range data1 {
			var find1 []*Tag1
			ok(t, store.Find(&find1, badgerdb.Where(badgerdb.Key).Eq(data1[i].ID)))
		...
		for i := range data2 {
			var find2 []*Tag2
			ok(t, store.Find(&find2, badgerdb.Where(badgerdb.Key).Eq(data2[i].ID)))
		...
		for i := range data3 {
			var find3 []*Tag3
			ok(t, store.Find(&find3, badgerdb.Where(badgerdb.Key).Eq(data3[i].ID)))
		...
		for i := range data4 {
			var find4 []*Tag4
			ok(t, store.Find(&find4, badgerdb.Where(badgerdb.Key).Eq(data4[i].ID)))
