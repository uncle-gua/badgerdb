...
var sortTests = []test{
	{
		name:   "Sort By Name",
		query:  badgerdb.Where("Category").Eq("animal").SortBy("Name"),
		result: []int{9, 5, 14, 8, 13, 2, 16},
	},
	{
		name:   "Sort By Name Reversed",
		query:  badgerdb.Where("Category").Eq("animal").SortBy("Name").Reverse(),
		result: []int{16, 2, 13, 8, 14, 5, 9},
	},
	{
		name:   "Sort By Multiple Fields",
		query:  badgerdb.Where("ID").In(8, 3, 13).SortBy("Category", "Name"),
		result: []int{13, 15, 4, 3},
	},
	{
		name:   "Sort By Multiple Fields Reversed",
		query:  badgerdb.Where("ID").In(8, 3, 13).SortBy("Category", "Name").Reverse(),
		result: []int{3, 4, 15, 13},
	},
	{
		name:   "Sort By Duplicate Field Names",
		query:  badgerdb.Where("ID").In(8, 3, 13).SortBy("Category", "Name", "Category"),
		result: []int{13, 15, 4, 3},
	},
	{
		name:   "Sort By Name with limit",
		query:  badgerdb.Where("Category").Eq("animal").SortBy("Name").Limit(3),
		result: []int{9, 5, 14},
	},
	{
		name:   "Sort By Name with skip",
		query:  badgerdb.Where("Category").Eq("animal").SortBy("Name").Skip(3),
		result: []int{8, 13, 2, 16},
	},
	{
		name:   "Sort By Name with skip and limit",
		query:  badgerdb.Where("Category").Eq("animal").SortBy("Name").Skip(2).Limit(3),
		result: []int{14, 8, 13},
	},
	{
	...
	},
	{
		...
		query:  badgerdb.Where("Category").Eq("animal").SortBy("Name").Skip(4),
		result: []int{13, 2, 16},
	},
	{
	...
	},
	{
		...
		query:  badgerdb.Where("Category").Eq("animal").SortBy("Name").Skip(10),
		result: []int{},
	},
}
	...
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		for _, tst := range sortTests {
			t.Run(tst.name, func(t *testing.T) {
				...
				if err != nil {
					t.Fatalf("Error finding sort data from badgerdb: %s", err)
				}
	...
	for _, tst := range sortTests {
		t.Run(tst.name, func(t *testing.T) {
			testWrap(t, func(store *badgerdb.Store, t *testing.T) {
				...
				var result []ItemTest
				err = store.Find(&result, badgerdb.Where("UpdateIndex").Eq("updated index").And("UpdateField").Eq("updated"))
				if err != nil {
...
func TestSortOnKey(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		var result []ItemTest
		_ = store.Find(&result, badgerdb.Where("Name").Eq("blah").SortBy(badgerdb.Key))
	})
}
...
func TestSortedFindOnInvalidFieldName(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		err := store.Find(&result, badgerdb.Where("BadFieldName").Eq("test").SortBy("BadFieldName"))
		if err == nil {
...
func TestSortedFindWithNonSlicePtr(t *testing.T) {
	testWrap(t, func(store *badgerdb.Store, t *testing.T) {
		...
		var result []ItemTest
		_ = store.Find(result, badgerdb.Where("Name").Eq("blah").SortBy("Name"))
	})
}
