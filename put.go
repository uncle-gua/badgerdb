...
var ErrKeyExists = errors.New("this Key already exists in badgerdb for this type")
...
// sequence tells badgerdb to insert the key as the next sequence in the bucket
type sequence struct{}

// NextSequence is used to create a sequential key for inserts
// Inserts a uint64 as the key
// store.Insert(badgerdb.NextSequence(), data)
func NextSequence() interface{} {
	return sequence{}
}

// Insert inserts the passed in data into the badgerdb
//
// If the key already exists in the badgerdb, then an ErrKeyExists is returned
// If the data struct has a field tagged as `badgerdbKey` and it is the same type
...
//
// To use this with badgerdb.NextSequence() use a type of `uint64` for the key field.
...
// Update updates an existing record in the badgerdb
...
// Upsert inserts the record into the badgerdb if it doesn't exist.  If it does already exist, then it updates
// the existing record
